...
{
    NSLog(@"TGCache: move \"%@\" -> \"%@\"", fileUrl, cacheUrl);
    dispatch_block_t block = ^
    {
...
- (void)clearCache:(int)availability
{
    ...
    if (availability & TGCacheDisk)
    {
        dispatch_async([TGCache diskCacheQueue], ^
        {
            ...
            NSLog(@"TGCache: removed %d files (%d failed)", removedCount, failedCount);
        });
    }
}
...
- (void)restoreMemoryCache:(NSArray *)array
{
    ...
    NSLog(@"Cache restored in %f ms", (CFAbsoluteTimeGetCurrent() - startTime) * 1000.0);
}
